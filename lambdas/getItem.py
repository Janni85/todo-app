...
def lambda_handler(event, context):
    try:
        ...
        if todo_id:  # Wenn TodoID angegeben ist, nur die Aufgabe mit der TodoID zurückgeben
            response = table.query(
                KeyConditionExpression=boto3.dynamodb.conditions.Key('UserID').eq(
                    user_id) & boto3.dynamodb.conditions.Key('TodoID').eq(todo_id)
            )
        ...
        tasks = [{'Task': item.get('Task'), 'TodoID': item.get(
            'TodoID')} for item in items if 'Task' in item and 'TodoID' in item]

        headers = {
            'Content-Type': 'application/json',
            # Hier kannst du auch die genaue Ursprungsdomain angeben, wenn bekannt
